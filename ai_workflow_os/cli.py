...
from .app_index import list_generated_apps, export_app_to_downloads
...
def cmd_doctor(args):
    ...
    required = [
        "README.md",
        "ai_workflow_os/core.py",
        "ai_workflow_os/server.py",
        "ai_workflow_os/cli.py",
        "ai_workflow_os/app_builder.py",
        "ai_workflow_os/app_index.py",
        "ai_workflow_os/android_builder.py",
        "web/index.html",
        "web/static/app.js",
        "web/static/app.css",
    ]
...
def cmd_providers(args):
    dump({"providers": list_providers()})
    return 0


def cmd_permissions(args):
    dump(default_policy_json())
    return 0


def cmd_tools(args):
    dump(tool_inventory() if tool_inventory else {"tools_module": False})
    return 0


def cmd_runs(args):
    dump({"runs": list_runs(Path.cwd()) if list_runs else []})
    return 0


def cmd_status(args):
    dump(scan_project(Path(args.target)).__dict__)
    return 0


def cmd_inspect(args):
    dump(RepoInspector().inspect(Path(args.target)) if RepoInspector else {"inspect_module": False})
    return 0


def cmd_secret_status(args):
    dump(secret_status(args.secret, project_id=args.project))
    return 0


def cmd_gh_status(args):
    dump(GhTool().status(Path.cwd(), execute=args.execute).to_dict() if GhTool else {"gh_tool": False})
    return 0


def cmd_gcloud_project(args):
    dump(GcloudTool().config_project(execute=args.execute).to_dict() if GcloudTool else {"gcloud_tool": False})
    return 0


def cmd_research_graph(args):
    dump(MarketResearchGraph().build_seed_graph(args.prompt))
    return 0


def cmd_research_queries(args):
    dump(build_research_queries(args.prompt, max_depth=args.depth))
    return 0


def cmd_list_apps(args):
    dump(list_generated_apps(Path.cwd()))
    return 0


def cmd_android_status(args):
    dump(android_status())
    return 0


def cmd_init_project(args):
    dump(install_into_project(Path(args.target), app_name=args.name))
    return 0
...
def cmd_create_app(args):
    dump(create_generated_app(args.prompt, Path(args.target), name=args.name, execute=args.execute))
    return 0


def cmd_test_app(args):
    dump(test_generated_app(Path(args.target)))
    return 0


def cmd_export_app_downloads(args):
    dump(export_app_to_downloads(Path.cwd(), args.name, Path.home() / "storage/downloads"))
    return 0


def cmd_android_target(args):
    dump(create_native_android_target(Path.cwd(), args.name, args.prompt))
    return 0


def cmd_self_bootstrap(args):
    dump({"self_bootstrap_plan": str(save_self_bootstrap_plan(Path.cwd(), args.prompt, Path(args.out)))})
    return 0
...
def cmd_approve(args):
    ...
    dump({"approval_file": str(path), "run_id": args.run_id})
    return 0
...
def cmd_agent_run(args):
    ...
    dump(execute_plan(Path(args.plan), approve=args.approve, approval_file=approval_file))
    return 0
...
def build_parser():
    ...
    p = sub.add_parser("list-apps"); p.set_defaults(func=cmd_list_apps)
    p = sub.add_parser("export-app-downloads"); p.add_argument("name"); p.set_defaults(func=cmd_export_app_downloads)

    p = sub.add_parser("android-status"); p.set_defaults(func=cmd_android_status)
    p = sub.add_parser("android-target"); p.add_argument("name"); p.add_argument("prompt"); p.set_defaults(func=cmd_android_target)
